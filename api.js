...
// 🔴 IMPORTANTE: POST /api/upload-album
app.post('/api/upload-album', async (req, res) => {
  try {
    const { albumName, photos } = req.body;
    ...
    res.json({
      success: true,
      message: `Álbum "${albumName}" creado y ${uploadResults.filter(r => r.success !== false).length}/${photos.length} fotos subidas`,
